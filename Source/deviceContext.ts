...
export class DeviceContext implements IDeviceContext, vscode.Disposable {
	...
	private _suppressSaveContext = false;
	...
	private constructor() {
		if (vscode.workspace && ArduinoWorkspace.rootPath) {
			...
			this._vscodeWatcher = vscode.workspace.createFileSystemWatcher(
				path.join(ArduinoWorkspace.rootPath, ".vscode"),
				true,
				true,
				false,
			);
	...
	public loadContext(): Thenable<object> {
		return vscode.workspace.findFiles(ARDUINO_CONFIG_FILE, null, 1).then(
			(files) => {
	...
	public async initialize() {
		if (
			...
			if (this.sketch) {
				...
				vscode.window.showInformationMessage(
					"No sketch (*.ino or *.cpp) was found or selected - initialization skipped.",
				);
			}
		}
	}
