...
export function validateArduinoPath(
	arduinoPath: string,
	useArduinoCli = false
): boolean {
	return fileExistsSync(
		path.join(
			resolveMacArduinoAppPath(arduinoPath, useArduinoCli),
			useArduinoCli ? "arduino-cli" : "/Contents/MacOS/Arduino"
		)
	);
}
...
export function findFile(fileName: string, cwd: string): string {
	...
	try {
		...
		if (
			pathString &&
			pathString[0] &&
			fileExistsSync(pathString[0].trim())
		) {
