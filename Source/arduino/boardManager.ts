...
import type { ArduinoApp } from "./arduino";
...
export class BoardManager {
	...
	constructor(
		...
		this._boardConfigStatusBar = vscode.window.createStatusBarItem(
	...
	public async loadPackages(update = false) {
		this._packages = [];
		this._platforms = [];
		...
		for (const indexFile of indexFiles) {
			...
			if (
				!update &&
				!util.fileExistsSync(
					path.join(rootPackageFolder, indexFileName),
				)
			) {
				await this.setPreferenceUrls(additionalUrls);
				await this._arduinoApp.initialize(true);
	...
	public async changeBoardType() {
		...
		const chosen = await vscode.window.showQuickPick(
			<vscode.QuickPickItem[]>supportedBoardTypes
				.map((entry): vscode.QuickPickItem => {
	...
	public getInstalledPlatforms(): any[] {
		...
		const mergePlatform = (plat) => {
			const find = installedPlatforms.find((_plat) => {
				return (
					_plat.packageName === plat.packageName &&
	...
	public loadPackageContent(indexFile: string): void {
		...
		try {
			...
			arduinoChannel.error(`Invalid json file "${path.join(
				...
				if (addedPlatform) {
					...
					if (
						...
						addedPlatform.versions[
							addedPlatform.versions.length - 1
						]
					) {
	...
	private updateStatusBar(show = true): void {
		if (show) {
			this._boardConfigStatusBar.show();
	...
	private loadInstalledPlatforms() {
		...
		installed.forEach((platform) => {
			const existingPlatform = this._platforms.find((_plat) => {
				return (
					_plat.package.name === platform.packageName &&
	...
	private getDefaultPlatforms(): IPlatform[] {
		...
		try {
			const packageBundled = fs.readFileSync(
				path.join(
					this._settings.defaultPackagePath,
	...
	private loadInstalledProgrammersFromPlatform(plat: IPlatform) {
		if (
			...
			const programmersContent = fs.readFileSync(
				path.join(plat.rootBoardPath, "programmers.txt"),
				"utf8",
			);
