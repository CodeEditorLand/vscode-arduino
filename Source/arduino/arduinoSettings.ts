...
export class ArduinoSettings implements IArduinoSettings {
	...
	private _usingBundledArduinoCli = false;
		...
		if (platform === "win32") {
			...
			if (
				util.directoryExistsSync(
					path.join(this._arduinoPath, "portable"),
				)
			) {
			...
			if (this.preferences.get("sketchbook.path")) {
				if (
					...
					this._sketchbookPath = path.join(
						this._arduinoPath,
						"portable",
						this.preferences.get("sketchbook.path"),
					);
				} else {
					this._sketchbookPath =
						this.preferences.get("sketchbook.path");
				}
			} else {
	...
	public get commandPath(): string {
		...
		if (platform === "darwin" && !this._usingBundledArduinoCli) {
			return path.join(
				...
				path.normalize(this._commandPath),
			);
		} else {
			return path.join(
				this._arduinoPath,
	...
	private async updateWindowsPath(): Promise<void> {
		...
		if (
			...
			util.fileExistsSync(
				path.join(this._arduinoPath, "AppxManifest.xml"),
			)
		) {
				...
				if (!isExecutable) {
					try {
						...
						Logger.notifyUserError(
							"arduino-cli-not-executable",
							e,
							messages.ARDUINO_CLI_NOT_EXECUTABLE +
								" " +
								bundledPath,
						);
					}
