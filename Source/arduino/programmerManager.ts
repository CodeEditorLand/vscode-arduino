...
import type { IBoard, IProgrammer } from "./package";

export class ProgrammerManager {
	public static notFoundDisplayValue = "<Select Programmer>";
	...
	public async selectProgrammer() {
		const selectionItems = this.getAvailableProgrammers(
			this._arduinoApp.boardManager.currentBoard,
		).map((programmer) => ({
