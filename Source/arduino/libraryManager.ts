...
export class LibraryManager {
	...
	public async loadLibraries(update = false) {
		this._libraryMap = new Map<string, ILibrary>();
		this._libraries = [];

		const libraryIndexFilePath = path.join(
		...
		await this.loadInstalledLibraries(
			path.join(this._settings.sketchbookPath, "libraries"),
			false,
		);
	...
	private async loadInstalledLibraries(libRoot: string, isBuiltin: boolean) {
		...
		for (const libDir of installedLibDirs) {
			...
			sourceLib.srcPath = util.directoryExistsSync(sourceLib.srcPath)
				? sourceLib.srcPath
				: path.join(libRoot, libDir);
		}
	}
	...
	private async parseBoardLibraries(
		rootBoardPath,
		architecture,
		librarySet: Set<any>,
	) {
		...
		if (util.directoryExistsSync(builtInLibPath)) {
			...
			for (const libDir of libDirs) {
				...
				sourceLib.srcPath = util.directoryExistsSync(sourceLib.srcPath)
					? sourceLib.srcPath
					: path.join(builtInLibPath, libDir);
