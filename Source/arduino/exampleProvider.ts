...
import type { ExampleManager, IExampleNode } from "./exampleManager";

export class ExampleProvider implements vscode.TreeDataProvider<ExampleItem> {
	private _onDidChangeTreeData: vscode.EventEmitter<null> =
	...
	private hasAnyExamples(nodes?: IExampleNode[]): boolean {
		return (
			nodes &&
			nodes.find(
				(node) => node.isLeaf || this.hasAnyExamples(node.children),
			) !== undefined
		);
	}
