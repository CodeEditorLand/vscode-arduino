...
export class ArduinoContentProvider
	implements vscode.TextDocumentContentProvider
{
	...
	public async initialize() {
		...
		this.addHandlerWithLogger(
			"open-settings",
			"/api/opensettings",
			(req, res) => this.openSettings(req, res),
			true,
		);
		...
		this.addHandlerWithLogger(
			"add-libpath",
			"/api/addlibpath",
			async (req, res) => await this.addLibPath(req, res),
			true,
		);
		...
		this.addHandlerWithLogger(
			"run-command",
			"/api/runcommand",
			(req, res) => this.runCommand(req, res),
			true,
		);
		...
		this.addHandlerWithLogger(
			"open-example",
			"/api/openexample",
			(req, res) => this.openExample(req, res),
			true,
		);
	...
	public async provideTextDocumentContent(uri: vscode.Uri): Promise<string> {
		...
		if (uri.toString() === Constants.BOARD_MANAGER_URI.toString()) {
			...
			uri.toString() === Constants.LIBRARY_MANAGER_URI.toString()
		) {
			type = "librarymanager";
                ...
                window.onload = function() {
                    ...
                    var url = "${(
						await vscode.env.asExternalUri(
							this._webserver.getEndpointUri(type),
						)
					).toString()}?" +
	...
	public getHtmlView(req, res) {
		return res.sendFile(
			path.join(this._extensionPath, "./out/views/index.html"),
		);
	}
	...
	public async installPackage(req, res) {
		if (!req.body.packageName || !req.body.arch) {
			return res
				.status(400)
				.send("BAD Request! Missing { packageName, arch } parameters!");
		} else {
			try {
				await ArduinoContext.arduinoApp.installBoard(
				...
				return res
					.status(500)
					.send(
						`Install board failed with message "code:${error.code}, err:${error.stderr}"`,
					);
			}
		}
	}
	...
	public async uninstallPackage(req, res) {
		if (req.body.packagePath) {
			try {
				await ArduinoContext.arduinoApp.uninstallBoard(
					req.body.boardName,
				...
				return res
					.status(500)
					.send(`Uninstall board failed with message "${error}"`);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { packagePath } parameter!");
		}
	}
	...
	public async openLink(req, res) {
		if (req.body.link) {
			try {
				...
				return res
					.status(500)
					.send(`Cannot open the link with error message "${error}"`);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { link } parameter!");
		}
	}
		...
		if (req.body.query) {
			try {
				...
				return res
					.status(500)
					.send(
						`Cannot open the setting with error message "${error}"`,
					);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { query } parameter!");
		}
	}
	...
	public async runCommand(req, res) {
		if (req.body.command) {
			try {
				...
				return res
					.status(500)
					.send(`Cannot run command with error message "${error}"`);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { command } parameter!");
		}
	}
	...
	public async installLibrary(req, res) {
		if (req.body.libraryName) {
			try {
				await ArduinoContext.arduinoApp.installLibrary(
				...
				return res
					.status(500)
					.send(
						`Install library failed with message "code:${error.code}, err:${error.stderr}"`,
					);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { libraryName } parameters!");
		}
	}
	...
	public async uninstallLibrary(req, res) {
		if (req.body.libraryPath) {
			try {
				await ArduinoContext.arduinoApp.uninstallLibrary(
					req.body.libraryName,
				...
				return res
					.status(500)
					.send(
						`Uninstall library failed with message "code:${error.code}, err:${error.stderr}"`,
					);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { libraryPath } parameters!");
		}
	}
		...
		if (req.body.libraryPath) {
			try {
				...
				return res
					.status(500)
					.send(
						`Add library path failed with message "code:${error.code}, err:${error.stderr}"`,
					);
			}
		} else {
			return res
	...
	public async getBoardConfig(req, res) {
		return res.json({
			configitems:
				ArduinoContext.boardManager.currentBoard === null
					? null
					: ArduinoContext.boardManager.currentBoard.configItems,
		});
	}
		...
		if (req.body.boardId) {
			try {
				...
				return res
					.status(500)
					.send(
						`Update board config failed with message "code:${error.code}, err:${error.stderr}"`,
					);
			}
		} else {
	...
	public async updateConfig(req, res) {
		if (!req.body.configId || !req.body.optionId) {
			...
			try {
				ArduinoContext.boardManager.currentBoard.updateConfig(
					req.body.configId,
					req.body.optionId,
				);
		...
		if (req.body.examplePath) {
			try {
				...
				return res
					.status(500)
					.send(
						`Cannot open the example folder with error message "${error}"`,
					);
			}
		} else {
			return res
				.status(400)
				.send("BAD Request! Missing { examplePath } parameter!");
		}
	}

	private addHandlerWithLogger(
		handlerName: string,
		url: string,
		...
		const wrappedHandler = async (req, res) => {
			...
			try {
				...
				Logger.traceError("expressHandlerError", error, {
					correlationId: guid,
					handlerName,
					...properties,
				});
