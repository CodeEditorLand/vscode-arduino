...
import {
	BoardConfigResult,
	type IBoard,
	type IBoardConfigItem,
	type IPlatform,
} from "./package";

export function parseBoardDescriptor(
	boardDescriptor: string,
	plat: IPlatform,
): Map<string, IBoard> {
...
export class Board implements IBoard {
	...
	public getBuildConfig() {
		return `${this.getPackageName()}:${this.platform.architecture}:${
			this.board
	...
	public get customConfig(): string {
		if (this._configItems && this._configItems.length > 0) {
			return this._configItems
				.map(
					(configItem) =>
						`${configItem.id}=${configItem.selectedOption}`,
				)
				.join(",");
		}
	}
	...
	public getPackageName() {
		return this.platform.packageName
			? this.platform.packageName
			: this.platform.package.name;
	}
}
 ...
 */
export function boardEqual(a: IBoard | undefined, b: IBoard | undefined) {
	if (a && b) {
		return a.key === b.key;
	} else if (a || b) {
		return false;
