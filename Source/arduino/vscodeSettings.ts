...
export class VscodeSettings implements IVscodeSettings {
	...
	public setCommandPath(value: string): Promise<void> {
		return this.setConfigValue(
			configKeys.ARDUINO_COMMAND_PATH,
			value,
			true,
		);
	}
	...
	public get additionalUrls(): string[] {
		const value = this.getConfigValue<string | string[]>(
			configKeys.ADDITIONAL_URLS,
		);
	...
	public get disableIntelliSenseAutoGen(): boolean {
		return this.getConfigValue<boolean>(
			configKeys.DISABLE_INTELLISENSE_AUTO_GEN,
		);
	}
	...
	public get analyzeOnSettingChange(): boolean {
		return this.getConfigValue<boolean>(
			configKeys.ANALYZE_ON_SETTING_CHANGE,
		);
	}
	...
	private async setConfigValue(key: string, value, global = true) {
		const workspaceConfig = vscode.workspace.getConfiguration();
