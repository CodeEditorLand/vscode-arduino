...
import type { IArduinoSettings } from "./arduinoSettings";
import type { BoardManager } from "./boardManager";
import type { ExampleManager } from "./exampleManager";
import {
	AnalysisManager,
	isCompilerParserEnabled,
	makeCompilerParserContext,
} from "./intellisense";
...
import type { ProgrammerManager } from "./programmerManager";
...
export class ArduinoApp {
	...
	constructor(private _settings: IArduinoSettings) {
		...
		this._analysisManager = new AnalysisManager(
			() => this._building,
			async () => {
				await this.build(BuildMode.Analyze);
			},
			analysisDelayMs,
		);
	}
	...
	public async initialize(force = false) {
		...
		if (
			force ||
			!util.fileExistsSync(
				path.join(this._settings.packagePath, "package_index.json"),
			)
		) {
			try {
	 ...
	 */
	public async initializeLibrary(force = false) {
		if (
			force ||
			!util.fileExistsSync(
				path.join(this._settings.packagePath, "library_index.json"),
			)
		) {
			try {
		...
		return await this._build(buildMode, buildDir)
			...
			.catch((reason) => {
				...
				logger.notifyUserError(
					"ArduinoApp.build",
					reason,
					`Unhandled exception when cleaning up build "${buildMode}": ${JSON.stringify(
						reason,
					)}`,
				);
				return false;
	...
	public async includeLibrary(libraryPath: string) {
		...
		if (util.fileExistsSync(appPath)) {
			...
			const activeEditor = vscode.window.visibleTextEditors.find(
				(textEditor) => {
					return (
						path.resolve(textEditor.document.fileName) ===
	...
	public async installBoard(
		...
		try {
			if (this.useArduinoCli()) {
				if (updatingIndex) {
					await this.spawnCli(["core", "update-index"], undefined, {
			...
			if (updatingIndex) {
				...
				arduinoChannel.end(
					`Installed board package - ${packageName}${os.EOL}`,
				);
			}
		} catch (error) {
	...
	public async installLibrary(
		...
		try {
			if (this.useArduinoCli()) {
				if (updatingIndex) {
					this.spawnCli(["lib", "update-index"], undefined, {
						channel: showOutput
			...
			if (error.code === 1) {
				if (updatingIndex) {
					...
					arduinoChannel.end(
						`Installed library - ${libName}${os.EOL}`,
					);
				}
			} else {
				arduinoChannel.error(`Exit with code=${error.code}${os.EOL}`);
	...
	public openExample(example) {
		function tmpName(name) {
			...
			while (true) {
				if (
					!util.fileExistsSync(candidateName) &&
	 ...
	 */
	protected async runPrePostBuildCommand(
		dc: DeviceContext,
		environment: any,
		what: "pre" | "post",
	): Promise<boolean> {
		const cmdline = what === "pre" ? dc.prebuild : dc.postbuild;
	...
	private isMemoryUsageInformation(line: string) {
		return (
			line.startsWith("Sketch uses ") ||
			line.startsWith("Global variables use ")
		);
	}
	...
	private async _build(
		...
		if (buildDir || dc.output) {
			...
			arduinoChannel.info(
				`Please see the build logs in output path: ${buildDir}`,
			);
		} else {
			const msg =
		...
		return await run(
			...
			{ cwd: ArduinoWorkspace.rootPath },
			{
				/*channel: arduinoChannel.channel,*/ stdout: stdoutcb,
				stderr: stderrcb,
			},
		).then(
			async () => {
