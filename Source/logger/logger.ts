	...
	winston.configure({
		transports: [
			new winston.transports.File({
				level: LogLevel.Warn,
				filename: context.asAbsolutePath("arduino.log"),
			}),
...
export function notifyAndThrowUserError(
	errorCode: string,
	error: Error,
	message?: string,
) {
...
export function notifyUserWarning(
	errorCode: string,
	error: Error,
	message?: string,
) {
	traceWarning(errorCode, error, {
		notification: message || error.message,
		showUser: true,
		telemetry: true,
	});
