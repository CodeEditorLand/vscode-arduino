...
export default class BoardConfigItemView extends React.Component<
	IBoardConfigItemProps,
	IBoardConfigItemState
> {
	constructor(props) {
		super(props);
		this.state = {
	...
	public render() {
		...
		return (
			<div class="listitem theme-listitem">
				<Col class="left-side d-inline-block" xs={3} sm={2} md={2}>
					{this.props.configitem.displayName}:
				</Col>
				<Col>
					<DropdownButton
						id={this.props.configitem.id}
						title={this.getOptionDisplayName()}
						...
						{options.map((opt, index) => {
							return (
								<MenuItem
									...
									eventKey={opt.id}
									active={
										opt.id === this.state.selectedOption
									}>
									{opt.displayName}
								</MenuItem>
							);
	...
	private getOptionDisplayName() {
		return this.props.configitem.options.find(
			(opt) => opt.id === this.state.selectedOption,
		).displayName;
	}
