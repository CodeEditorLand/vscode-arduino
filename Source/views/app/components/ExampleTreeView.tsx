...
class ExampleTreeView extends React.Component<
	IExampleViewProps,
	IExampleViewState
> {
			...
			if (example.children && example.children.length) {
				return (
					<Tree.TreeNode
						...
						title={example.name}
						isLeaf={false}
						expanded={!!this.state[example.path]}>
	...
	private openExample(selectedKeys, event) {
		if (event.node.props.isLeaf) {
			...
			const index = this.state.expandedKeys.indexOf(
				event.node.props.eventKey,
			);
