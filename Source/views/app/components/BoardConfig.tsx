...
class BoardConfig extends React.Component<IBoardConfigProps, React.Props<any>> {
	...
	public render() {
		...
		if (this.props.installedBoards.length === 0) {
			installWarning = (
				<div className="theme-bgcolor arduinomanager-toolbar">
					No boards are installed.{" "}
					<a
						className="help-link"
						onClick={() =>
							API.runCommand("arduino.showBoardManager")
						}>
						Install a board with the Boards Manager.
					</a>
				</div>
			);
		}
				...
				<div className="boardConfig">
					<Grid fluid>
						<Row key="board-selector">
							<BoardSelector
								installedBoards={this.props.installedBoards}
								loadConfigItems={this.props.loadConfigItems}
							/>
						</Row>
						{this.props.configitems.map((configitem, index) => {
							return (
								<Row key={configitem.id}>
									<BoardConfigItemView
