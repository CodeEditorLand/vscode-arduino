...
const mapDispatchToProps = (dispatch) => {
	return {
		loadBoardPackages: (update = false) =>
			actions.getBoardPackages(dispatch, update),
		installBoard: (boardName, packageName, arch, version) =>
			actions.installBoard(
				dispatch,
				boardName,
				packageName,
				arch,
		...
		uninstallBoard: (boardName, packagePath) =>
			actions.uninstallBoard(dispatch, boardName, packagePath, () => {
				actions.getBoardPackages(dispatch, false);
			}),
	};
};
...
> {
	...
	public render() {
		...
		const filteredPlatforms = this.props.platforms.filter((element) => {
			return (
				filterType(element, this.state.category) &&
		...
		const isOperating =
			!!this.props.installingBoardName ||
			!!this.props.uninstallingBoardName;
		return (
			<div class={"boardmanager " + (isOperating ? "disabled" : "")}>
				...
				<div class="arduinomanager-toolbar theme-bgcolor">
					...
					<Button
						class="operation-btn"
						bsStyle="link"
						onClick={() => this.props.loadBoardPackages(true)}>
				...
				<div class="arduinomanager-footer theme-bgcolor">
					...
					<a
						class="help-link right-side"
						title="Configure Additional Boards Manager URLs"
						onClick={() =>
							API.openSettings("arduino.additionalUrls")
						}>
						Additional URLs
					</a>
				</div>
