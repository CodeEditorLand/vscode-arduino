...
> {
	constructor(props) {
		super(props);
		this.state = {
			selectedBoard: this.props.installedBoards.find(
				(bd) => bd.isSelected,
			),
		};
	...
	public componentWillReceiveProps(nextProps) {
		if (!this.state.selectedBoard && nextProps.installedBoards.length) {
			this.setState({
				selectedBoard: nextProps.installedBoards.find(
					(bd) => bd.isSelected,
				),
			});
		}
	...
	public render() {
		...
		return (
			<div class="listitem theme-listitem">
				...
				<Col class="react-selector btn-group">
					<Select
						clearable={false}
						name="select-board"
						options={options}
