...
> {
	constructor(props) {
		super(props);
		this.state = {
			version: props.platform.installedVersion
				? ""
				: props.platform.versions[0],
		};
	...
	public render() {
		return (
			<div class="listitem theme-listitem">
	...
	private buildBoardSectionHeader(p) {
		return (
			<div>
				<span class="listitem-header">{p.name}</span>
				{p.defaultPlatform && <span> Built-In </span>}
				<span class="listitem-author">
					{" "}
					by{" "}
					<span class="listitem-header">{p.package.maintainer}</span>
				</span>
				{p.installedVersion && (
					<span class="listitem-author">
						{" "}
						Version {p.installedVersion}{" "}
						<span class="listitem-installed-header">INSTALLED</span>
					</span>
				)}
			</div>
		);
		...
		return (
			<div class="listitem-container">
				<div>
					...
					<br /> {p.boards.map((board: any) => board.name).join(", ")}
				</div>
				{helpLinks.map((helpLink, index) => {
					return (
						<div key={index}>
							<a
								class="help-link"
								onClick={() => this.openLink(helpLink.url)}>
								{helpLink.label}
							</a>
						</div>
					);
	...
	private buildBoardSecionButtons(p) {
		return (
			<div class="listitem-footer">
				{this.props.installingBoardName === p.name && (
					<div class="toolbar-mask theme-bgcolor">Installing...</div>
				)}
				{this.props.uninstallingBoardName === p.name && (
					<div class="toolbar-mask theme-bgcolor">Removing...</div>
				)}
				{p.installedVersion && (
					<div class="right-side">
						{p.versions &&
							p.versions.length &&
							versionCompare(p.versions[0], p.installedVersion) >
								0 && (
								<Button
									class="operation-btn"
									onClick={() =>
										this.props.installBoard(
											p.name,
											p.package.name,
											p.architecture,
											p.versions[0],
				...
				{p.versions && p.versions.length > 1 && (
					<div class="left-side">
						<DropdownButton
							id="versionselector"
							title={this.state.version || "Select version"}
							placeholder="Select version"
							onSelect={this.versionUpdate}>
