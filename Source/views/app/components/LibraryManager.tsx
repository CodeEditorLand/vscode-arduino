...
const mapStateToProps = (store) => {
	return {
		...
		uninstallingLibraryName:
			store.libraryManagerStore.uninstallingLibraryName,
	};
};
...
const mapDispatchToProps = (dispatch) => {
	return {
		loadLibraries: (update = false) =>
			actions.getLibraries(dispatch, update),
		installLibrary: (libraryName, version, callback) =>
			actions.installLibrary(dispatch, libraryName, version, (error) => {
				if (error) {
					callback();
				} else {
		...
		uninstallLibrary: (libraryName, libraryPath, callback) =>
			actions.uninstallLibrary(
				dispatch,
				libraryName,
				libraryPath,
				(error) => {
...
class LibraryManager extends React.Component<
	ILibraryManagerProps,
	ILibraryManagerState
> {
	constructor(props) {
		super(props);
		this.state = {
	...
	public render() {
			...
			switch (type) {
				...
				default:
					return (
						...
						element.types.some((ele) => ele === type)
					);
			}
		}
		...
		const filteredLibraries = this.props.libraries.filter((element) => {
			const filterSupported = this.state.checked
				? element.supported
				: true;
			if (
				filterSupported &&
				filterType(element, this.state.type) &&
				filterTopic(element, this.state.topic) &&
		...
		const isOperating =
			!!this.props.installingLibraryName ||
			!!this.props.uninstallingLibraryName;
		...
		const itemRenderer = (index, key) => {
			...
			return (
				<LibraryItemView
					key={filteredLibraries[index].name}
		...
		return (
			<div class={"librarymanager " + (isOperating ? "disabled" : "")}>
				{this.props.requesting && (
					<div class="mask theme-bgcolor">Loading...</div>
				)}
				<div class="arduinomanager-toolbar theme-bgcolor">
					<div class="dropdown-filter">
						<span class="dropdown-label">Type</span>
						<DropdownButton
							id="typeselector"
							title={this.state.type}
							onSelect={this.typeUpdate}>
							{this.props.types.map((c, index) => {
								return (
									<MenuItem
										key={index}
										eventKey={c}
										active={c === this.state.type}>
										{c}
									</MenuItem>
								);
							})}
					...
					<div class="dropdown-filter">
						<span class="dropdown-label">Topic</span>
						<DropdownButton
							id="topicselector"
							title={this.state.topic}
							onSelect={this.topicUpdate}>
							{this.props.categories.map((c, index) => {
								return (
									<MenuItem
										key={index}
										eventKey={c}
										active={c === this.state.topic}>
										{c}
									</MenuItem>
								);
							})}
					...
					/>
					<Checkbox
						class="supported-checkbox"
						onChange={this.handleCheck}>
						Only show libraries supported by current board
					</Checkbox>
					<Button
						class="operation-btn"
						bsStyle="link"
						onClick={() => this.props.loadLibraries(true)}>
						Refresh Library Index
					</Button>
				</div>
				<div class="arduinomanager-container">
					<ReactList
						itemRenderer={itemRenderer}
						itemSizeEstimator={itemSizeEstimator}
						length={filteredLibraries.length}
						type="variable"
					/>
				</div>
				<div class="arduinomanager-footer theme-bgcolor">
