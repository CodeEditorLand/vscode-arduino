...
export class UsbDetector {
			...
			if (device.vendorId && device.productId) {
				...
				if (
					...
					(util.fileExistsSync(
						...
						(openEditor &&
							openEditor.document.fileName.endsWith(".ino")))
				) {
					if (!bd) {
						ArduinoContext.boardManager
							.updatePackageIndex(deviceDescriptor.indexFile)
							.then((shouldLoadPackageContent) => {
								const ignoreBoards =
								...
								vscode.window
									.showInformationMessage(
										`Install board package for ${deviceDescriptor.name}`,
										"Yes",
										"No",
										"Don't ask again",
									)
									.then((ans) => {
										if (ans === "Yes") {
											ArduinoContext.arduinoApp
												...
												.then(() => {
													if (
														shouldLoadPackageContent
													) {
														ArduinoContext.boardManager.loadPackageContent(
	...
	private switchBoard(bd: IBoard, deviceDescriptor, showReadMe = true) {
