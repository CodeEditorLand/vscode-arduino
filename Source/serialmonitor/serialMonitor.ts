...
export class SerialMonitor implements vscode.Disposable {
	public static DEFAULT_TIMESTAMP_FORMAT = "";
	...
	private lastSelectedBaudRate = 115200; // Same default as Arduino.
	...
	public async initialize(extensionContext: vscode.ExtensionContext) {
		...
		this.portsStatusBar = vscode.window.createStatusBarItem(
		...
		this.timestampFormatStatusBar = vscode.window.createStatusBarItem(
			vscode.StatusBarAlignment.Right,
		...
		this.serialMonitorApi = await getSerialMonitorApi(
			Version.latest,
			extensionContext,
		);
	...
	public async selectSerialPort(): Promise<string | undefined> {
		...
		const chosen = await vscode.window.showQuickPick(
			<vscode.QuickPickItem[]>ports
				.map((l: PortInformation): vscode.QuickPickItem => {
					return {
	...
	public async selectBaudRate(): Promise<number | undefined> {
		...
		if (!Number.parseInt(chosen, 10)) {
			Logger.warn("Serial Monitor has not been started");
			return undefined;
		}
		const selectedRate: number = Number.parseInt(chosen, 10);
		this.lastSelectedBaudRate = selectedRate;
		return selectedRate;
	}

	public async openSerialMonitor(restore = false): Promise<void> {
		this.checkForUndefinedSerialMonitorApi(true);
		...
		const baudRate = restore
			? this.lastSelectedBaudRate
			: (await this.selectBaudRate()) ?? this.lastSelectedBaudRate;
	...
	private checkForUndefinedSerialMonitorApi(showError = false): void {
		const errorString =
