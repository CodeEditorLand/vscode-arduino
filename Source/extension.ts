...
const arduinoContentProviderModule = impor(
	"./arduino/arduinoContentProvider",
) as typeof import("./arduino/arduinoContentProvider");
import type { IBoard } from "./arduino/package";
...
const completionProviderModule = impor(
	"./langService/completionProvider",
) as typeof import("./langService/completionProvider");
import { BuildMode } from "./arduino/arduino";
import * as Logger from "./logger/logger";
import { SerialMonitor } from "./serialmonitor/serialMonitor";
const usbDetectorModule = impor(
	"./serialmonitor/usbDetector",
) as typeof import("./serialmonitor/usbDetector");
...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (openEditor && openEditor.document.fileName.endsWith(".ino")) {
		...
		if (
			...
			vscode.window.showWarningMessage(
				`The open file "${workingFile}" is not inside the workspace folder, ` +
	...
	const commandExecution = async (
		...
		try {
			...
			Logger.traceError("executeCommandError", error, {
				correlationId: guid,
				command,
			});
		}
	...
	if (!vscodeSettings.useArduinoCli) {
		...
		void askSwitchToBundledCli(
			constants.messages.REMOVE_ARDUINO_IDE_SUPPORT +
				" " +
				constants.messages.SWITCH_TO_BUNDLED_CLI,
		);
	}

	const registerArduinoCommand = (
		command: string,
		commandBody: (...args: any[]) => any,
		getUserData?: () => any,
	): number => {
		return context.subscriptions.push(
			vscode.commands.registerCommand(command, async (...args: any[]) => {
				...
				if (
					...
					Logger.traceError(
						"InvalidArduinoPath",
						new Error(constants.messages.INVALID_ARDUINO_PATH),
					);
					await askSwitchToBundledCli(
						constants.messages.INVALID_ARDUINO_PATH +
							" " +
							constants.messages.SWITCH_TO_BUNDLED_CLI,
					);
	...
	const registerNonArduinoCommand = (
		command: string,
		commandBody: (...args: any[]) => any,
		getUserData?: () => any,
	): number => {
	...
	registerArduinoCommand(
		"arduino.initialize",
		async () => await deviceContext.initialize(),
	);
	...
	registerArduinoCommand(
		...
		async () => {
			if (!arduinoContextModule.default.arduinoApp.building) {
				await vscode.window.withProgress(
					...
					async () => {
						await arduinoContextModule.default.arduinoApp.build(
							BuildMode.Verify,
						);
					},
				);
		...
		() => {
			return {
				board:
					arduinoContextModule.default.boardManager.currentBoard ===
					null
						? null
						: arduinoContextModule.default.boardManager.currentBoard
								.name,
	...
	registerArduinoCommand(
		...
		() => {
			return {
				board: arduinoContextModule.default.boardManager.currentBoard
					.name,
			};
		},
	);
	...
	registerArduinoCommand("arduino.selectSketch", async () => {
		...
		const excludePattern = `{${excludePatterns
			.map((p) => p.replace("\\", "/"))
			.join(",")}}`;
	...
	registerArduinoCommand(
		...
		async (packageName, arch, version = "") => {
			let installed = false;
	...
	vscode.window.onDidChangeActiveTextEditor(async () => {
		...
		if (
			...
			((path.basename(activeEditor.document.fileName) ===
				"arduino.json" &&
				path.basename(path.dirname(activeEditor.document.fileName)) ===
					".vscode") ||
	...
	setTimeout(async () => {
		...
		registerArduinoCommand(
			...
			() => {
				return {
					board: arduinoContextModule.default.boardManager
						.currentBoard.name,
				};
			},
		);
		registerArduinoCommand(
			...
			() => {
				return {
					board:
						arduinoContextModule.default.boardManager
							.currentBoard === null
							? null
							: arduinoContextModule.default.boardManager
									.currentBoard.name,
