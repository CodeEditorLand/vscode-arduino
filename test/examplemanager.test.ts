...
suite("Arduino: Example Manager.", () => {
	...
	test("should be able to load examples", function (done) {
		...
		arduinoSettings
			.setup((x) => x.defaultExamplePath)
			.returns(() => Resources.mockedIDEExamplesPath);
		...
		exampleManager
			...
			.then((examples) => {
				...
				assert.equal(
					examples[1].children[0].children[0].name,
					"WebServer"
				);
				assert.equal(
					examples[1].children[0].children[0].path,
					Path.join(
						Resources.mockedIDELibPath,
						"Ethernet",
						"examples",
						"WebServer"
					)
				...
				assert.equal(
					...
					"Verify Examples from Custom Libraries"
				);
				...
				assert.equal(
					examples[2].children[0].children[0].name,
					"simplesample_http"
				);
				assert.equal(
					examples[2].children[0].children[0].path,
					Path.join(
