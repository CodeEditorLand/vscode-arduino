...
suite("Arduino: App Initialization", () => {
	...
	test("should be able to resolve arduino settings correctly", function (done) {
		...
		assert.equal(
			util.directoryExistsSync(arduinoSettings.arduinoPath),
			true,
			"should resolve arduino installation directory automatically"
		);

		assert.equal(
			util.fileExistsSync(arduinoSettings.commandPath),
			true,
			"should resolve arduino executable correctly"
		);

		assert.equal(
			util.directoryExistsSync(arduinoSettings.defaultPackagePath),
			true,
			"should resolve arduino IDE built-in package directory correctly"
		);

		assert.equal(
			util.directoryExistsSync(arduinoSettings.defaultLibPath),
			true,
			"should resolve arduino IDE built-in library directory correctly"
		);

		assert.equal(
			util.directoryExistsSync(arduinoSettings.defaultExamplePath),
			true,
	...
	test("should be able to download necessary package_index and preferences.txt", function (done) {
		const arduinoSettings = ArduinoContext.arduinoApp.settings;
		assert.equal(
			util.fileExistsSync(arduinoSettings.preferencePath),
			true,
			"should be able to init preferences.txt file if not found"
		);

		assert.equal(
			arduinoSettings.preferences.get("sketchbook.path"),
			arduinoSettings.sketchbookPath
		);

		assert.equal(
			util.fileExistsSync(
				Path.join(arduinoSettings.packagePath, "package_index.json")
			),
			true,
