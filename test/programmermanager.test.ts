...
suite("Arduino: Programmer Manager.", () => {
	...
	setup((done) => {
		...
		mockBoardManager
			.setup((x) => x.currentBoard)
			.returns(() => mockedBoard.object);
		mockBoardManager
			.setup((x) => x.installedProgrammers)
			.returns(() => installedProgrammers);
		mockBoardManager
			.setup((x) => x.onBoardTypeChanged)
			.returns(() => new vscode.EventEmitter<void>().event);

		// Mock minimal ArduinoApp
		const arduinoApp = TypeMoq.Mock.ofType(ArduinoApp);
		arduinoApp
			.setup((x) => x.boardManager)
			.returns(() => mockBoardManager.object);

		try {
			programmerManager = new ProgrammerManager(
				TypeMoq.Mock.ofType(ArduinoSettings).object,
				arduinoApp.object
			);
			done();
		} catch (error) {
	...
	test("changing arduino.ino value should change programmer", function (done) {
		...
		setTimeout(() => {
			...
			assert.equal(
				programmerManager.currentDisplayName,
				programmers[0].displayName
			);
			done();
		}, 200);
	});
