...
const usbDetectorModule = impor(
	"../src/serialmonitor/usbDetector"
) as typeof import("../src/serialmonitor/usbDetector");

// Defines a Mocha test suite to group tests of similar kind together
suite("Arduino: Extension Tests", () => {
	test("should be present", () => {
		assert.ok(
			vscode.extensions.getExtension("vsciot-vscode.vscode-arduino")
		);
	});
	...
	test("should be able to register arduino commands", () => {
		return vscode.commands.getCommands(true).then((commands) => {
			...
			const foundArduinoCommands = commands.filter((value) => {
				return (
					ARDUINO_COMMANDS.indexOf(value) >= 0 ||
					value.startsWith("arduino.")
				);
			});

			const errorMsg =
				"Some Arduino commands are not registered properly or a new command is not added to the test";
			assert.equal(
				foundArduinoCommands.length,
				ARDUINO_COMMANDS.length,
				errorMsg
			);
		});
	});
