...
suite("Arduino: Library Manager.", () => {
	...
	test("should be able to load libraries", function (done) {
		...
		arduinoSettings
			.setup((x) => x.defaultLibPath)
			.returns(() => Resources.mockedIDELibPath);
		arduinoSettings
		...
		mockedBoardManager
			.setup((x) => x.getInstalledPlatforms())
			.returns(() => [
				{
					rootBoardPath: Path.join(
		...
		arduinoApp
			.setup((x) => x.boardManager)
			.returns(() => mockedBoardManager.object);
		...
		libraryManager
			...
			.then(() => {
				...
				assert.equal(
					libraries.length,
					945,
					"Library Manager should display all libraries listed in library_index.json"
				);
				...
				assert.equal(
					...
					4,
					"Library Manager should display installed libraries"
				);
				assert.equal(installedLibraries[0].name, "Ethernet");
				assert.equal(installedLibraries[0].builtIn, true);
				assert.equal(
					installedLibraries[0].srcPath,
					Path.join(Resources.mockedIDELibPath, "Ethernet", "src"),
			...
			ArduinoContext.arduinoApp
				...
				.then((result) => {
					...
					if (util.directoryExistsSync(libPath)) {
							...
							new Error(
								"FastLED library install failure, can't find library path: " +
									libPath
							)
						);
					}
	...
	test("should be able to remove libraries", () => {
		try {
			...
			if (util.directoryExistsSync(libPath)) {
				...
				assert.equal(
					util.directoryExistsSync(libPath),
					false,
