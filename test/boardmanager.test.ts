...
suite("Arduino: Board Manager.", () => {
	...
	setup(function (done) {
		...
		arduinoSettings
			.setup((x) => x.preferences)
			.returns(() => new Map<string, string>());
		arduinoSettings
			.setup((x) => x.packagePath)
			.returns(() => Resources.mockedPackagePath);
		arduinoSettings
			.setup((x) => x.defaultPackagePath)
			.returns(() => Resources.mockedIDEPackagePath);
		arduinoSettings
			.setup((x) => x.sketchbookPath)
			.returns(() => Resources.mockedSketchbookPath);

		const arduinoApp = TypeMoq.Mock.ofType(ArduinoApp);
		arduinoApp.setup((x) =>
			x.setPref(TypeMoq.It.isAny(), TypeMoq.It.isAny())
		);
		arduinoApp.setup((x) => x.initialize(TypeMoq.It.isAny()));

		try {
			boardManager = new BoardManager(
				arduinoSettings.object,
				arduinoApp.object
			);
			boardManager
				.loadPackages(false)
	...
	test("should be able to load packages", () => {
		...
		assert.equal(
			platforms[0].name,
			"Arduino AVR Boards",
			"Board Manager should display built Arduino AVR Boards"
		);
		assert.equal(
			platforms[0].installedVersion,
			"1.6.18",
			"Arduino IDE built-in AVR board package version should be 1.6.18"
		);
		assert.equal(
			platforms[0].rootBoardPath,
			Path.join(Resources.mockedIDEPackagePath, "arduino", "avr"),
	...
	test("should be able to load installed platforms", () => {
		...
		assert.equal(
			...
			1,
			"should display installed platforms via board manager."
		);

		const totalInstalledPlatforms = boardManager.getInstalledPlatforms();
		assert.equal(
			totalInstalledPlatforms.length,
			2,
			"should parse installed platforms from board manager and manaually downloaded packages"
		);
	});

	test("should be able to load installed boards", () => {
		assert.equal(
			boardManager.installedBoards.size,
			46,
			"Arduino IDE should contains built-in AVR boards"
		);
		assert.equal(
			boardManager.installedBoards.has("arduino:avr:yun"),
			true,
			"should parse installed boards from Arduino IDE built-in packages"
		);
		assert.equal(
			boardManager.installedBoards.has("esp8266:esp8266:huzzah"),
			true,
	...
	test("should be able to load installed programmers", () => {
		assert.equal(
			boardManager.installedProgrammers.size,
			17,
			`Expected to find programmers for dummy & AVR boards`
		);
		assert.ok(
			boardManager.installedProgrammers.get("avrispmkii"),
			"should parse installed programmers from Arduino IDE built-in packages"
		);
		assert.ok(
			boardManager.installedProgrammers.get("esp8266_dummy"),
	...
	test("should parse boards.txt correctly", () => {
		const arduinoAvrBoard = fs.readFileSync(
			Path.join(Resources.mockedIDEPackagePath, "arduino/avr/boards.txt"),
			"utf8"
		);
		const platform = {
		...
		const boardDescriptor = parseBoardDescriptor(
			arduinoAvrBoard,
			<IPlatform>platform
		);
		...
		assert.equal(
			diecimilaBoard.configItems.length,
			1,
			"should parse config items from boards.txt correctly"
		);
	...
	test("should parse programmers.txt correctly", () => {
		const arduinoAvrBoard = fs.readFileSync(
			Path.join(
				Resources.mockedIDEPackagePath,
		...
		const programmerDescriptors = parseProgrammerDescriptor(
			arduinoAvrBoard,
			<IPlatform>platform
		);
	...
	test("should parse platform.txt correctly", () => {
		const platformConfig = util.parseConfigFile(
			Path.join(
				Resources.mockedSketchbookPath,
				"hardware/esp8266/esp8266/platform.txt"
			)
		);
	...
	test("should be able to install boards packages", function (done) {
		...
		try {
			...
			ArduinoContext.arduinoApp
				.installBoard("Microsoft", "win10", "1.1.2", true)
				.then((result) => {
					const arduinoSettings = ArduinoContext.arduinoApp.settings;
					const packagePath = Path.join(
						arduinoSettings.packagePath,
						"packages",
						"Microsoft"
					);
					...
					if (util.directoryExistsSync(packagePath)) {
							...
							new Error(
								"Microsoft board package install failure, can't find package path :" +
									packagePath
							)
						);
					}
	...
	test("should be able to remove boards packages", () => {
		try {
			...
			if (util.directoryExistsSync(packagePath)) {
				ArduinoContext.arduinoApp.uninstallBoard(
					"Microsoft",
					packagePath
				);
				assert.equal(
					util.directoryExistsSync(packagePath),
					false,
